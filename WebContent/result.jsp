...
<body>
<% 
   Integer currentPagess=(Integer)request.getAttribute("currentPage");
   Integer totalrowsss=(Integer)request.getAttribute("totalrows");
   int currentPage=currentPagess.intValue();
   int totalrows=totalrowsss.intValue();
   PageBean aaa=new PageBean(currentPage,totalrows);
   String search=(String)request.getAttribute("searchType");
   String keyword=(String)request.getAttribute("search_text");
   Integer hottestids=(Integer)request.getAttribute("hottestid");
   Integer highids=(Integer)request.getAttribute("highid");
   int hottestid=hottestids.intValue();
   int highid=highids.intValue();
%>
<%
	  ...
	  <div class="col-md-10 sap_tabs">	
	       <div id="horizontalTab" style="display: block; width: 100%; margin: 0px;">
			  <ul class="resp-tabs-list">
			  	  <li class="resp-tab-item" aria-controls="tab_item-0" role="tab"><span>Normal</span></li>
				  <li class="resp-tab-item" aria-controls="tab_item-1" role="tab"><span>Hottest</span></li>
				  <li class="resp-tab-item" aria-controls="tab_item-2" role="tab"><span>HighScore</span></li>
				   ...
				   <div class="tab-1 resp-tab-content" aria-labelledby="tab_item-0">
						<ul class="tab_img">
						  <% 
						   ...
						   <li><a href="SearchDetailServlet?id=<%=filmList.get(i).getId()%>&currentPage=<%=currentPage %>&totalrows=<%=totalrows %>&rank=<%=i %>&keyword=<%=keyword %>&hottestid=<%=hottestid %>&highid=<%=highid %>">
				     ...
				     <div class="tab-1 resp-tab-content" aria-labelledby="tab_item-1">
						<ul class="tab_img">
						  <% 
						   ...
						   filmListhot=(List<filmBean>)request.getAttribute("filmlisthot");
						   System.out.println("this step is ok!");
						   for(int i=0;i<filmListhot.size();i++){%>
						   <li><a href="SearchDetailServlet?id=<%=filmListhot.get(i).getId()%>&currentPage=<%=currentPage %>&totalrows=<%=totalrows %>&rank=<%=i %>&keyword=<%=keyword %>&hottestid=<%=hottestid %>&highid=<%=highid %>">
							 <img src="images/photodatabase/graph/<%=filmListhot.get(i).getPhotoid() %>" class="img-responsive" onerror=this.src='images/default.jpg' alt="<%=filmListhot.get(i).getName() %>"/>
						     <div class="tab_desc">
							   <p><div style="width: 100px; background: #ccc; overflow: hidden; white-space: nowrap;  text-overflow: ellipsis; "><%=filmListhot.get(i).getName() %></div></p>
								<h4><%=filmListhot.get(i).getId() %></h4>
						  </div>
					      </a></li>
					      <%} %>
						   <div class="clearfix"></div>
				     ...
				     <div class="tab-1 resp-tab-content" aria-labelledby="tab_item-2">
						<ul class="tab_img">
						  <% 
						   ...
						   filmListhigh=(List<filmBean>)request.getAttribute("filmlisthigh");
						   System.out.println("this step is ok!");
						   for(int i=0;i<filmListhot.size();i++){%>
						   <li><a href="SearchDetailServlet?id=<%=filmListhigh.get(i).getId()%>&currentPage=<%=currentPage %>&totalrows=<%=totalrows %>&rank=<%=i %>&keyword=<%=keyword %>&hottestid=<%=hottestid %>&highid=<%=highid %>">
							 <img src="images/photodatabase/graph/<%=filmListhigh.get(i).getPhotoid() %>" class="img-responsive" onerror=this.src='images/default.jpg' alt="<%=filmListhigh.get(i).getName() %>"/>
						     <div class="tab_desc">
							   <p><div style="width: 100px; background: #ccc; overflow: hidden; white-space: nowrap;  text-overflow: ellipsis; "><%=filmListhigh.get(i).getName() %></div></p>
								<h4><%=filmListhigh.get(i).getId() %></h4>
						  </div>
					      </a></li>
					      <%} %>
						   <div class="clearfix"></div>
           ...
           <ul class="pagination">
	        <li>
	        <% if(aaa.isHasPreviousPage()){%>
	          <a href="AllinfoPageServlet?searchType=<%=search%>&currentPage=<%=currentPage-1 %>&field=<%=field %>&search_text=<%=keyword %>&totalrows=<%=totalrows %>&hottestid=<%=hottestid %>&highid=<%=highid %>" aria-label="Previous">
	            <span aria-hidden="true">«</span>
	          </a>
	          <li><a href="AllinfoPageServlet?searchType=<%=search%>&currentPage=<%=currentPage-1 %>&field=<%=field %>&search_text=<%=keyword %>&totalrows=<%=totalrows %>&hottestid=<%=hottestid %>&highid=<%=highid %>"><%=aaa.getCurrentPage()-1%></a></li>
	          <%} %>
	        </li>
	        <li><a href="#"><%=aaa.getCurrentPage()%></a></li>
	        <% if(aaa.isHasNextPage()){%>
	        <li><a href="AllinfoPageServlet?searchType=<%=search%>&currentPage=<%=currentPage+1 %>&field=<%=field %>&search_text=<%=keyword %>&totalrows=<%=totalrows %>&hottestid=<%=hottestid %>&highid=<%=highid %>"><%=aaa.getCurrentPage()+1%></a></li>
	        <li>
	          <a href="AllinfoPageServlet?searchType=<%=search%>&currentPage=<%=currentPage+1 %>&field=<%=field %>&search_text=<%=keyword %>&totalrows=<%=totalrows %>&hottestid=<%=hottestid %>&highid=<%=highid %>" aria-label="Next">
	            <span aria-hidden="true">»</span>
	          </a>
	        </li>
	        <%} }else{%>
	        ...
	        <li>
	        <% if(aaa.isHasPreviousPage()){%>
	          <a href="AllinfoPageServlet?searchType=<%=search%>&currentPage=<%=currentPage-1 %>&search_text=<%=keyword %>&totalrows=<%=totalrows %>&hottestid=<%=hottestid %>&highid=<%=highid %>" aria-label="Previous">
	            <span aria-hidden="true">«</span>
	          </a>
	          <li><a href="AllinfoPageServlet?searchType=<%=search%>&currentPage=<%=currentPage-1 %>&search_text=<%=keyword %>&totalrows=<%=totalrows %>&hottestid=<%=hottestid %>&highid=<%=highid %>"><%=aaa.getCurrentPage()-1%></a></li>
	          <%} %>
	        </li>
	        <li><a href="#"><%=aaa.getCurrentPage()%></a></li>
	        <% if(aaa.isHasNextPage()){%>
	        <li><a href="AllinfoPageServlet?searchType=<%=search%>&currentPage=<%=currentPage+1 %>&search_text=<%=keyword %>&totalrows=<%=totalrows %>&hottestid=<%=hottestid %>&highid=<%=highid %>"><%=aaa.getCurrentPage()+1%></a></li>
	        <li>
	          <a href="AllinfoPageServlet?searchType=<%=search%>&currentPage=<%=currentPage+1 %>&search_text=<%=keyword %>&totalrows=<%=totalrows %>&hottestid=<%=hottestid %>&highid=<%=highid %>" aria-label="Next">
	            <span aria-hidden="true">»</span>
	          </a>
	        </li>
	         <%}} %>	        
