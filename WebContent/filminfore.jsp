...
<body>
<%
    String currentPagess=(String)request.getAttribute("currentPage");
    String totalrowsss=(String)request.getAttribute("totalrows");
    String ranksss=(String)request.getAttribute("rank");
    int currentPage=Integer.parseInt(currentPagess);
    int totalrows=Integer.parseInt(totalrowsss);
    int rank=Integer.parseInt(ranksss);
    String keyword=(String)request.getAttribute("keyword");
    filmBean hottest=(filmBean)request.getAttribute("hottest");
    filmBean highest=(filmBean)request.getAttribute("highest");
    int rankness=(currentPage-1)*15+rank+1;
		   ...
		   <div class="single_box1">
			 ...
			 <div class="col-sm-7 col_6">
				<ul class="size">
		            ...
		            <% }%> 
		            <li>The rank of the film is <%=rankness %> in <%=totalrows %> films</li>  
				</ul>
			</div>
			<div class="clearfix"> </div>
			<div>			      
			      <ul class="tab_img">
						  <% 
						   if(film.getId()!=hottest.getId()){%>
						   <li>The hottest film while searching <%=keyword %> is shown below:</li></br>
						   <li><a href="SearchDetailServlet?id=<%=hottest.getId()%>&currentPage=<%=currentPage %>&totalrows=<%=totalrows %>&rank=<%=rank %>&keyword=<%=keyword %>&hottestid=<%=hottest.getId() %>&highid=<%=highest.getId() %>">
							 <img src="images/photodatabase/graph/<%=hottest.getPhotoid() %>" class="img-responsive" onerror=this.src='images/default.jpg' alt="<%=hottest.getName() %>"/>
						     <div class="tab_desc">
							   <p><div style="width: 100px; background: #ccc; overflow: hidden; white-space: nowrap;  text-overflow: ellipsis; "><%=hottest.getName() %></div></p>
								<h4><%=hottest.getId() %></h4>
						  </div>
					      </a></li>
					      <%} %>
						...
						<ul class="tab_img">
						  <% 
						   if(film.getId()!=highest.getId()){%>
						   <li>The film with highest score while searching <%=keyword %> is shown below:</li></br>
						   <li><a href="SearchDetailServlet?id=<%=highest.getId()%>&currentPage=<%=currentPage %>&totalrows=<%=totalrows %>&rank=<%=rank %>&keyword=<%=keyword %>&hottestid=<%=hottest.getId() %>&highid=<%=highest.getId() %>">
							 <img src="images/photodatabase/graph/<%=highest.getPhotoid() %>" class="img-responsive" onerror=this.src='images/default.jpg' alt="<%=highest.getName() %>"/>
						     <div class="tab_desc">
							   <p><div style="width: 100px; background: #ccc; overflow: hidden; white-space: nowrap;  text-overflow: ellipsis; "><%=highest.getName() %></div></p>
								<h4><%=highest.getId() %></h4>
						  </div>
					      </a></li>
					      <%} %>
